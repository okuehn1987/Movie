<script setup lang="ts">
import { Link, router } from '@inertiajs/vue3';

defineProps<{
    canResetPassword?: boolean;
    status?: string;
}>();

const adminForm = useForm({
    name: '',
    email: '',
    password: '',
    password_confirmation: '',
});

const adminLogin = () => {
    adminForm.post(route('register'), {
        onFinish: () => {
            adminForm.reset('password');
            if (!adminForm.errors.email && !adminForm.errors.password) {
                router.reload();
            }
        },
    });
};
</script>

<template>
    <GuestLayout title="Login">
        <v-form @submit.prevent="adminLogin">
            <v-row>
                <v-col cols="12">
                    <v-text-field
                        v-model="adminForm.name"
                        :readonly="adminForm.processing"
                        :error-messages="adminForm.errors.name"
                        prepend-inner-icon="mdi-account-outline"
                        label="Name"
                    ></v-text-field>
                    <v-text-field
                        v-model="adminForm.email"
                        :readonly="adminForm.processing"
                        :error-messages="adminForm.errors.email"
                        prepend-inner-icon="mdi-email-outline"
                        label="Email"
                    ></v-text-field>
                    <v-text-field
                        v-model="adminForm.password"
                        :readonly="adminForm.processing"
                        :error-messages="adminForm.errors.password"
                        type="password"
                        prepend-inner-icon="mdi-lock-outline"
                        label="Passwort"
                        class="mt-2 mb-n3"
                    ></v-text-field>
                    <v-text-field
                        v-model="adminForm.password_confirmation"
                        :readonly="adminForm.processing"
                        :error-messages="adminForm.errors.password_confirmation"
                        type="password"
                        prepend-inner-icon="mdi-lock-outline"
                        label="Passwort bestätigen"
                        class="mt-2 mb-n3"
                    ></v-text-field>
                    <!-- <v-checkbox label="Angemeldet bleiben" v-model="adminForm.remember">
                    <!-- <v-checkbox label="Angemeldet bleiben" v-model="adminForm.remember">
                    </v-checkbox> -->
                </v-col>
                <v-col cols="12">
                    <v-btn :loading="adminForm.processing" block color="primary" type="submit" size="large">Registrieren</v-btn>
                </v-col>

                <v-col cols="12">
                    <Link :href="route('password.request')" class="text-primary text-decoration-none">
                        <v-icon icon="mdi-lock"></v-icon>
                        Passwort vergessen?
                    </Link>
                </v-col>
            </v-row>
        </v-form>
    </GuestLayout>
</template>
<style scoped></style>
