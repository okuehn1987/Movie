...
const editableWorkLogs = computed(() =>
    props.paginator.data.filter((_, i) => i < 5)
);
...
function submit() {
    workLogForm
        ...
        .patch(route("workLog.update", { workLog: workLogForm.id }), {
            onSuccess: () => {
                workLogForm.reset();
...
function editWorkLog(id: WorkLog["id"]) {
    const workLog = props.paginator.data.find((e) => e.id === id);
    if (!workLog) return;

    const lastPatch = workLog.work_log_patches.at(-1);
    if (lastPatch && lastPatch.status === "created")
        showPatch.value = lastPatch;
...
function retreatPatch() {
    const workLog = props.paginator.data.find((e) => e.id === workLogForm.id);
    if (!workLog) return;
    ...
    <AdminLayout :title="user.first_name + ' ' + user.last_name">
        <v-container>
            <v-alert
                class="mb-4"
                color="primary"
                v-if="workLogForm.wasSuccessful"
                closable
            >
                Korrigierung der Arbeitszeit erfolgreich beantragt.
            </v-alert>
            <v-data-table
                v-model:page="paginator.current_page"
                :headers="[
                ...
                :items="
                    paginator.data
                        ...
                        .map((workLog) => ({
                            start: DateTime.fromSQL(workLog.start).toFormat(
                                'dd.MM.yyyy HH:mm'
                            ),
                            end: DateTime.fromSQL(workLog.end).toFormat(
                                'dd.MM.yyyy HH:mm'
                            ),
                            is_home_office: workLog.is_home_office
                                ? 'Ja'
                                : 'Nein',
                            id: workLog.id,
                            status: {
                                ...
                                paginator.data
                                    .find((e) => e.id === workLog.id)
                                    ?.work_log_patches.at(-1)?.status || 'none'
                            ],
                        }))
                "
            >
                <template v-slot:item.action="{ item }">
                    ...
                    >
                        <v-icon
                            :icon="
                                paginator.data
                                    .find((log) => log.id === item.id)
                                    ?.work_log_patches.at(-1)?.status ===
                                'created'
                                    ? 'mdi-eye'
                                    : 'mdi-pencil'
                            "
                        ...
                        <v-row no-gutters>
                            ...
                            <v-col cols="12" md="3">
                                <v-checkbox
                                    ...
                                    required
                                    :error-messages="
                                        workLogForm.errors.is_home_office
                                    "
